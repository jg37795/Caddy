window.GreenDetect.detect = function (data) {
  ...
  for (const cells of comps) {
    ...
    console.error('[dbg] COMP area', area.toFixed(0), 'cd', cd.toFixed(1),
  ...
  let _c = 0; for (let i = 0; i < N; i++) if (chosen[i]) _c++;
  console.error("[dbg] chosen cells:", _c, "of", N, "| chosen[oi]:", chosen[oi]);
