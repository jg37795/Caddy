...
const CACHE_VERSION = 'v1.20.0'; // TRUE-SCALE HOLE MAP (James: 'Map the ponds bunker greens etc correctly! Like osm' + reference screenshot): the frame distortion was the last lie - Y was squashed to 22% of hole length and EVERYTHING CLAMPED into frame, so course-sized shapes (the exec donut pond) flattened and dragged over the hole while off-frame parts vanished. FIXES: (1) TRUE-SCALE UNIFORM FIT - one yards-per-pixel for X and Y, window height surveyed from the hole path + green + assigned shapes (capped 220 yd so a huge shape can't flatten the hole), NO CLAMPING anywhere - oversized shapes simply clip at the frame edge like any honest map; (2) ASSIGNMENT CORRIDOR 160->90 yd - only shapes genuinely in play from the tee bind to the hole (the neighbour donut stops stamping onto hole 1); (3) RENDER PROOF on REAL exec hole 1 data (33 OSM elements live): donut pond with hollow centre left-of-centre, green upper-right, two bunkers, tee box at the line start, straight flight line - matches the OSM reference screenshot in dark theme; proof script + PNG committed (.gtds/hole1_v120.png).
