...
const CACHE_VERSION = 'v1.19.0'; // REAL OSM SHAPES ON THE HOLE MAP (James, OSM-vs-Caddy screenshot, approved proposal): OSM already maps fairway/bunker/water/tee-box outlines and our import query already FETCHED them - we threw them away. Now: (1) IMPORT captures polygons (golf=fairway, golf=rough, golf=bunker, natural=water, golf=tee) via osmRing + osmSimplifyRing (<=14 pts each); (2) SPATIAL ASSIGNMENT - each polygon belongs to every hole whose path runs within 160 yd of its centroid (shared bunkers draw on both holes - truthful); (3) CARTOON draws rough underlay, real fairway polygons (band stays only as fallback when no fairway polys), water, tee boxes, real bunker outlines (dots only for point-only hazards) - muted fills in the app's dark palette (rough 38,66,48 / fairway 64,152,99 / water steel-blue / tee sage); (4) SATELLITE SHEET draws the same real polygons under the ribbon + real bunker outlines; (5) shapes exposed through holeInfo bridge + sheet payload; (6) RE-MAP COURSE button on the saved-course card re-runs the OSM import with the current pipeline (preserves player-placed tees, source=osm only) so existing courses upgrade; (7) E2E v190 7/7: capture, assignment, cartoon rendering, sheet payload, Re-map visibility.
