...
const CACHE_VERSION = 'v1.11.0'; // HOLE VIEW ROUND (James x4): (1) LENGTH ADAPTATION - corridor cap 300m->560m (covers 600yd; his 536yd hole no longer loses the tee half) and grid N scales with span (96/128/160) so resolution holds; (2) TAP-WHILE-LOADING RACE fixed - tapping Hole mid-green-load no longer half-activates the corridor dataset; loadCorridor's completion path is the single owner of activate+fit (the squished-corridor bug); resize handler also mode-routes (fitHoleView in hole view); (3) TEE SWITCHER in the 3D hole view dock - chips from the saved course (course= param from Prep's 3D button), persists activeTeeSet + tee memory so Prep/Round follow; (4) HILLSHADE - baked vertex colours modulated by LiDAR surface normals (NW sun, 0.75-1.18 band) so ridges read through the satellite photo; void cells keep flat light.
