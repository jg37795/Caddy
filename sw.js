...
const CACHE_VERSION = 'v1.4.0'; // SOLID DRUM + DOCK REDESIGN (approved plan): grid floor now paints BEFORE the wall (its translucent lines+pale disc were overlaying the drum from high cameras = the 'translucent base') + dark contact ring grounds the drum; legend moved TOP-LEFT (title follows mode); dock rebuilt as 3 contextual rows — view segment + exag / Slope|Elev segmented buttons + layers (2D only) / ONE ball toggle (Drop<->Remove) with Stimp chip visible only while a ball is down; layer row hidden in 3D/Hole
