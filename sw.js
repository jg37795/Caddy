...
const CACHE_VERSION = 'v1.4.1'; // DEVICE BUGS: (CRITICAL) satellite never loaded on PHONE — satview decoded tiles with Buffer.from() which does not exist in browsers; now browser path assigns the tile URL directly (Esri ACAO:*). (2) surface UNDERSIDE painted over the wall on falling flanks: undersides now classified + drawn as a dark underlay BEFORE the wall (wall covers them; they only fill AA seam gaps); undersides excluded from the dressing zbuf so wall occludes them properly. (3) contact ring follows the real projected base ring (was a screen-space ellipse that ignored rotation). (4) exag drift: render3D rebuilds when the mesh was built at a different exag than the slider (float-then-snap). (5) legend dropped below the full topbar (no more status-text overlap). (6) dock rows nowrap + taller buttons (spacing).
