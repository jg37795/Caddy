...
const CACHE_VERSION = 'v1.15.4'; // FAINT LINE + RICHER ADVICE + SPACING (James device round): (1) the faint straight tee-green line was the OSM hole WAY's RETURN LEG - some ways run green->tee->...->green (looped/extended); drawing the raw way drew the return leg across the band. Fix at import (trimToJourney: walk from the tee end, stop at the point closest to the green) AND defensively at draw time in prep holeMapSvg (slice pathPts at the green-nearest point) so already-saved courses heal without re-mapping. (2) Expanded shot spacing - .prep-num-inline gets padding rhythm + dashed divider before the tweaks. (3) RICHER per-shot advice: approach rows say Favor the left/right (LiDAR feed) + deep-green attack note; tee rows get dogleg direction + play-the-edge advice computed from the path midpoint vs chord; hazards-in-landing-window notes unchanged. (4) CONDITIONS ANSWER (code-verified, no change needed): Prep's Use-live-weather pulls api.weather() = Open-Meteo at the PLAY tab's GPS location, cached 10 min (WEATHER_TTL) - real conditions on course, home weather when prepping from home; elevation in Prep = USGS tee-green delta via the ELEV chip.
