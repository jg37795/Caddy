...
const CACHE_VERSION = 'v1.15.3'; // MAP POLISH + APPROACH FAVOR (James device round): (1) dashed chord REMOVED from the hole map (the fairway shape + yardage label say it all). (2) CLUB SEGMENTS RESTORED - James liked them; now colored by Bag-tab category (Driver gold/woods, irons blue, wedges purple) with landing dots as endpoints; labels in the same hue. (3) HOW-TO-PLAY-IT PILLS color-coordinate with the same bag hues (one classification helper clubCatOf + CLUB_CAT_HEX shared by map and pills). (4) APPROACH FAVOR: the last plan row (approach) now says which side of the green to favor from the LiDAR green brief - green feeds right ~5 in -> Favor the left side (high side); falls back to hazard/depth notes when no brief is loaded.
