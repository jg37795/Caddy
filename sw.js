...
const CACHE_VERSION = 'v1.18.0'; // SATELLITE SHEET: DIRECT PAYLOAD + DISPERSION + AUTO GREEN BRIEF + IN-SHEET TEE (James device round + discussed additions): (1) FIRST-TAP EMPTY SHEET FIXED - the sheet re-read the course from localStorage by courseId which was flaky (first tap: dots only; second tap: ribbon+ring but no dots, no tee). The caller now passes the hole's real geometry (pathPts, greenRingPts, teePoint, greenCenter, hazards w/ raw lat/lng now exposed by holeInfo) - everything loads on EVERY tap. (2) CARRY TAGS - landing tags showed from-tee path distance (Driver 194 / GW 387); they now show each shot's CARRY from _planShotYds (Driver 275, GW 112). (3) MOVE TEE = TAP-TO-PLACE ON THE SHEET - no navigation (the old deep-link opened the Verify-green editor whose Cancel revealed the 3D tool, never Prep). Two-tap: arm (yellow banner + Cancel) -> tap the map -> tee marker moves + saves teePoint/teeSource=player to the course profile; live rebind hook. 3D Green keeps its verified deep-link. (4) DISPERSION ELLIPSES - 1-sigma along/cross per club from the shot-log posterior (or prior), rotated to the shot bearing, in the club color, under each landing dot; dashed white target line ties tee -> landings -> green. New api.clubSigmas(name) bridge. (5) AUTO GREEN BRIEF (James: why rely on me opening 3D Green?) - new greenBriefCore.js runs the FULL pipeline headlessly (USGS 64x64 patch around the green -> computeGradientField -> simPuttPath 3-zone probes) and persists in greenmap's EXACT schema (caddy:greenBrief:v1) + slopePct/highSide additions; triggered on sheet open, 30-day freshness, one fetch per green; green ring tinted high-side bright / feed-side dim from it. (6) greenmap_smoke 6b check rewritten to nearest-point path sampling (monotonic-x lerp broke on bend-back fixtures).
