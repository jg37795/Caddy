...
const CACHE_VERSION = 'v1.16.1'; // DUPLICATES + SHEET ACTIONS + ADVICE BOX (James device round): (1) DUPLICATE PLAN ROWS - the v1.16 tee-box fold-in pushed the plan TWICE (once at the original push, again after mutating lines[0]); pushed once now. (2) Move tee + 3D Green buttons REMOVED from the card and now live INSIDE the satellite sheet (psh-actions row): Move tee deep-links Check location w/ armtee=1 (carrying lat/lng/teelat/teelng), 3D Green re-boots greenmap at this hole. (3) The sheet FITS THE HOLE: fitBounds over path+green+tee w/ 48px padding (hole centred, not pinned to the green at z17). (4) The green ADVICE BOX is restored after the plan (deep/shallow green, soft/firm turf, par-5 layup commitment, water-at-carry-zone callout) - James missed the overall advice.
