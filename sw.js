...
const CACHE_VERSION = 'v1.19.1'; // STRAIGHT SHOT LINES + MULTIPOLYGON WATER (James, exec hole 1 screenshot): (1) shot segments drew ALONG the OSM hole way (the ground centerline) so the 8-iron line followed the dogleg curve - but a golf shot FLIES; segments are now STRAIGHT lines tee -> landing -> green between the plan's split points (landing positions unchanged); on the par-3 the line honestly crosses the water corner. pathDbetween removed. (2) 'gap in the water': big water/fairway bodies are MULTIPOLYGON RELATIONS (several outer rings sharing edges); osmRing took only the FIRST outer member so the body drew as fragments. New osmOuterRings expands relations into ALL outer rings, each simplified <=14 pts and drawn - shared edges read as one continuous body. (3) E2E v190 extended: water fixture is now a 2-outer-ring relation (asserts 2 water rings captured) + straight-segment assertion (M x y L x y).
