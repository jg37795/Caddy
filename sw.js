...
const CACHE_VERSION = 'v1.15.2'; // PREP MAP + FLOW FINAL (James, screenshots + discussed): (1) STRAIGHT LINE - the colored shot lines WERE the straight tee-to-green look; removed entirely. The map is now: fairway band as the hero (brighter, 30px), one colored LANDING DOT per shot at its spot on the path colored by the club's Bag-tab category (woods gold/irons blue/wedges purple/putter green, classification mirrored from bag.js inferCat), small labels beside the dots, and a SUBTLE dashed tee-green reference under the band. (2) THE NUMBER BOX REMOVED - tapping a shot-plan row expands the full number INSIDE that row (plays-like, adjustments, carry/release/total, aim, lie/shape tweaks move with it); tapping again collapses; with no shot selected there is NO number UI at all (plan rows show every number at rest). NUMBER_TEMPLATE is a function (fresh lie/shape active states per injection); lie/shape handlers delegated on document; prepEffortTag folded into the reason line. paintControls guards for the moved rows.
