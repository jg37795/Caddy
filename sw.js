...
const CACHE_VERSION = 'v1.19.2'; // NO MORE CRACKS + WATER-CARRY AWARENESS (James, exec hole 1 follow-up): (1) TWO GAPS IN THE WATER - my index-resampling simplified adjacent water rings to DIFFERENT vertices along their shared edge, so shared borders no longer aligned (cracks). osmSimplifyRing is now proper Douglas-Peucker with iterative tolerance (4 m start, x1.6 until <= maxPts, cap 60 m) - shared nodes are geometrically necessary so they survive on BOTH rings and shared edges simplify identically. Unit-verified: two squares sharing a jagged edge keep both shared-edge endpoints after simplification. (2) DOES IT KNOW I'M HITTING OVER IT? - now yes: water-carry check walks the straight tee->pin line in 3 yd steps, point-in-polygon against the hole's water shapes; if crossed (>=8 yd run), the caddie read says 'Your line carries water from ~N yd out - take the club that clears ~M yd with room to spare' (M = far edge + 8). Leads the advice when present. E2E 9/9 incl. carry callout.
