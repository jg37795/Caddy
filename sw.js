...
const CACHE_VERSION = 'v1.20.3'; // THE LIKED VIEW RESTORED (James: 'still showing too much course' + 'previous view had the current hole centered' + pond slivers): the v1.20.1 hole-only window (140 yd cap) CLIPPED the donut pond into slivers - the pond is bigger than the cap. FIX: shapes expand the window again (crossSpan cap 200 yd) so the pond draws fully, while the v1.20.2 GRASS-corridor tightening (fairway/rough 55 yd, water/bunker 90 yd) keeps neighbour clutter out. NET: the liked previous view (full horseshoe pond, hole centered and dominant) minus the clutter. Brighter water styling (fill 0.55, stroke 0.8 @1.5) so the horseshoe arms read at cartoon scale. RENDER PROOF re-run on real exec hole 1 data: full horseshoe pond, flight line spanning the frame, two bunkers + neighbour greens only - hole1_v120.png re-rendered and verified.
