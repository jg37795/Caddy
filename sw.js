...
const CACHE_VERSION = 'v1.15.1'; // DOGLEG MIRROR FIX + TAP-A-SHOT (James device round 3): (1) the v1.15 mirror persisted - the v1.14.1 basis flip made +cross=golfer-right but the map's Y() mapping still SUBTRACTED cross (left-positive era), so right-of-play rendered ABOVE the line: dogleg left drawn as dogleg right, hazards mirrored. Y() now ADDS cross (viewer-behind-tee: golfer-right = below the line). E2E now asserts the DRAWN side matches the text side (bunker cy vs path y at its cx) - the assertion gap that let two mirrors ship. (2) TAP-A-SHOT: How-to-play-it rows are buttons now - tapping a shot loads it into THE NUMBER (carry for that shot's yardage, lie/shape/aim solve for it), tapping again returns to the tee number. planShotIdx + _planShotYds state; chosen row highlighted.
