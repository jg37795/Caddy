...
const CACHE_VERSION = 'v1.20.1'; // HOLE WINDOW + GRAZE PHRASING + ROUGH-AS-CORRIDOR (James: 'showing way too much of the course' / 'pond still incorrect' / 'some holes still old style'): (1) WINDOW = THE HOLE'S WINDOW - shape vertices no longer expand the map frame (they clip at the edge); crossSpan capped 220->140 yd; the hole fills the frame instead of compressing into the bottom third. (2) GRAZE vs CARRY: water crossings under 25 yd of run now phrase honestly - 'Your line clips the water corner from ~N yd out - clear ~M yd and it's never in play' (full-carry language only for real carries). (3) OLD-STYLE HOLES: the stroked band engaged whenever the course had no golf=FAIRWAY polygons - but the exec tags its light green as golf=ROUGH; when rough polygons exist they ARE the corridor (band only when neither fairway nor rough exists). (4) RENDER PROOF re-run on real exec hole 1 (green associated nearest path end): green at line end, donut pond grazing the line lower edge, bunkers/tees placed true - matches OSM reference.
