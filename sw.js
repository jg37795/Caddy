...
const CACHE_VERSION = 'v1.3.2'; // v1.3.1 REGRESSION KILLED + polish: (CRITICAL) rebuildMeshArrows indexed the ElevGrid OBJECT not the raw array — every 3D arrow was silently dropped ('arrows show nothing'); fixed to index .grid with W/H/cellSize locals; (2) flag/ball now have VISIBILITY FLOORS (13 CSS px pennant, 7 px ball) — the depth-proportional sizes were 3-4px on phones, indistinguishable from the old primitives; (3) exag slider debounced 140ms (a full mesh rebuild per tick was the jerk); (4) Check-location map maxZoom 19→21 (maxNativeZoom 19) — the hard zoom cap; (5) drill-down guard: tapping deep in the green zone shows the OSM polygon list first when >1 candidate green matches
