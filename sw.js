...
const CACHE_VERSION = 'v1.19.3'; // RENDER-PROOFED WATER ASSEMBLY (James: 'gotten worse - blocky shards' + 'how can I trust this'): this round shipped only after rendering the REAL Jester Park data through the new pipeline and visually comparing to the OSM reference. (1) ROOT CAUSE of the shards: OSM maps big lakes as relations whose outer members are ARCS OF ONE RING (member N ends where N+1 starts) - v1.19.1 closed each arc independently = chord fans. osmOuterRings now ASSEMBLES arcs: walk members, join shared endpoints (4-way match: tail/head x arc-start/arc-end, reversing as needed), close only when the walk returns to its start. Proven on relation 115195: 4 arcs (84 pts) -> ONE closed 81-pt ring. (2) BLOCKINESS: index budget 14 pts/4 m tol -> Douglas-Peucker 40 pts at 2 m tolerance (SIMPLIFY_TOL_M=2) - organic shorelines. (3) BOUNDING GATE: rings with bbox diagonal > 1200 yd are dropped (the 12-mi-away lake that clamped into the map frame via X()/Y() saturation - that was the shard-over-grass artifact). (4) RENDER PROOF: water_v193.png rendered from the real relation through the exact shipped code path - one continuous smooth lake matching OSM's own rendering of the donut-shaped pond (the hollow center is the real green peninsula). Suite: E2E v190 9/9, all suites green.
