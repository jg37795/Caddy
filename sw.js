...
const CACHE_VERSION = 'v1.9.0'; // PREP SINGLE HOLE CARD (James: rethink the tab): Pre-shot / The-shot / Conditions boxes merged into ONE hole card. Reading order: header -> map (ELEV chip) -> target carry TILES (now the picker: tap Front/Middle/Back = play it, chosen state moves, number re-solves in place) -> THE NUMBER (big, live, with adjustments + aim) -> tweaks (lie/shape inline) -> caddy notes -> Conditions collapsed inside the card (dial/temp/altitude/green-vs-ball/ground/live-sync). Unmapped holes: honest 'isn't mapped' state + 3D Green button + back, no dead boxes. Old prepRecCard/prepShotCard/prepCondCard markup deleted; prepGreenPicker replaced by tiles-as-picker. E2E .gtds/e2e_hole_card.js: 9/9 PASS.
