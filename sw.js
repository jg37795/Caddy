...
const CACHE_VERSION = 'v1.12.0'; // STYLIZED HOLE RENDER + TEE EDITOR (James, discussed): (1) Option B - hole view texture is now OUR LiDAR colours by default (Stylized: fairway/rough tints + green slope ramp + hillshade, zero satellite), with a Stylized/Photo toggle in the dock (Photo = the old satellite blend, persisted per device, re-bakes colours only). (2) TEE EDITING moved into Check location: 'Move tee' arms two-tap placement identical to Round's Set tee (tap map = place, tap tee = remove, draggable), 'Load this green' re-boots with BOTH green + tee and persists a manual teePoint per hole to the course profile so Prep/Round/hole-view agree. (3) DELETED: v1.11 dock tee chips (label-only), prep tee chips + nudge stepper + setTeeSet bridge (wrong abstraction - James wanted placement, not named sets).
