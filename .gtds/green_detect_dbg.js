window.GreenDetect.detect = function (data) {
  ...
  console.log("EXIT maxNear", maxNear); if (maxNear < 0.36) return null;
  ...
  console.log("EXIT !best"); if (!best) return null;
  ...
  console.log("EXIT !loops"); if (!loops.length) return null;
  const areaAbs = (p) => {
    let a = 0;
  ...
  console.log("EXIT simp len", simp.length); if (simp.length < 8 || simp.length > 64) return null;
  ...
  if (conf > 1) conf = 1;
  console.log("EXIT conf", conf); if (conf < 0.5) return null;
