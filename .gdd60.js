window.GreenDetect.detect = function (data) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const sTx = rampL(tx[i], pTx8, Math.max(pTx40, pTx8 * 1.8));
    ...
    // v-tune2b: hard veto only for true water (flat AND br<55);
  ...
  function grow(loT, hiT, maxR) {
    ...
    // v-tune3 (product contract): the pin IS on the green - always seed
  ...
  // v-r4 (RELATIVE gates): shaded greens (728: br p10 50) and steep
  ...
  const flankOk = (i) => {
    // v-r4b: relative to core AND loose absolute floor (absolute alone
    // failed shaded/steep greens; relative alone over-admits on 252).
    if (!fin(sm[i]) || sm[i] < coreSm - 0.10 || sm[i] > coreSm + 0.28) return false;
    if (fin(ex[i]) && ex[i] < coreEx - 30) return false;
    if (fin(br[i]) && br[i] < coreBr - 40) return false;
  ...
  // v-r3 (tracer repair): pick the ring that CONTAINS the blob centroid —
