window.GreenDetect.detect = function (data) {
  ...
  for (const cells of comps) {
    ...
    if (cd > 120) continue;
    console.log("COMP area", area.toFixed(0), "cd", cd.toFixed(1), "contains", contains, "bbox", (maxx-minx).toFixed(0) + "x" + (maxy-miny).toFixed(0)); const key = (contains ? 0 : 1) * 1e6 + cd + minD * 0.25;
