(() => {
  ...
  function renderHoleOverlay() {
    ...
    // Fairway polygons can be absent or imperfect in OSM.
    // Keep them subtle so they never obscure satellite imagery.
    const fairways = Array.isArray(hole.fairwayOutline)
      ? hole.fairwayOutline
      : [];

    for (const fairway of fairways) {
      addHoleOutline(layer, toLeafletLatLngs(fairway), {
        color: colors.fairway,
        weight: 1.5,
        opacity: isSatellite ? 0.45 : 0.65,
        fillColor: colors.fairway,
        fillOpacity: isSatellite ? 0.08 : 0.12,
      });
    }

    // Exact green polygon, when OSM supplied one.
    const greenOutline = toLeafletLatLngs(hole.greenOutline);

    if (greenOutline.length >= 3) {
      addHoleOutline(layer, greenOutline, {
        color: colors.green,
        weight: 2.5,
        opacity: 0.95,
        fillColor: colors.green,
        fillOpacity: isSatellite ? 0.20 : 0.28,
      });
    }
    ...
    if (state.markers.greenCenter?.bringToFront) {
      state.markers.greenCenter.bringToFront();
    }
  ...
  function applyHoleGeometryToMap() {
    ...
    // A hole can have mapped hazards/fairway even if it does not have a
    // usable green centre. Draw what OSM provided either way.
...
out geom;`;
  ...
  function buildCourseFromOSM(osmData, courseNode) {
    ...
    const holeFairwayMap = new Map();

    for (const fairway of fairways) {
      const coords = osmWayCoords(fairway);
      ...
      for (const [ref, holeWay] of holeMap) {
        const holeCoords = osmWayCoords(holeWay);
        const holeCentroid = polygonCentroid(holeCoords);

        if (!holeCentroid) continue;

        const distance = geodesicInverse(centroid, holeCentroid).s;
      ...
      if (bestHole !== null && bestDist < 600) {
        const current = holeFairwayMap.get(bestHole) || [];

        current.push(coords);

        holeFairwayMap.set(bestHole, current);
      }
    }
    ...
    for (const haz of allHazards) {
      ...
      if (bestHole !== null && bestDist < 400) {
        ...
        arr.push({
          ...
          // Preserve the actual OSM shape for rendering.
          outline: coords,
        });
    ...
    for (let i = 1; i <= 18; i++) {
      ...
      const fairwayOutline = holeFairwayMap.get(i) || [];

      const greenOutline = greenWay ? osmWayCoords(greenWay) : [];
