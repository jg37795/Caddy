window.GreenDetect.detect = function (data) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (hasSat) {
      ...
      let sBr = 0.5;
      // v-tune (my iteration): WATER/SHADOW VETO — dark smooth regions
      // (ponds, shadowed canopy) beat greens on smoothness; brightness
      // separates them. Bands: <70 = water/shadow, 90-190 = turf.
      const brV = br[i];
      if (brV === brV) {
        if (brV < 70) { sBr = 0; }             // water/shadow — hard veto
        else if (brV >= 90 && brV <= 190) sBr = 1;
        else sBr = 0.45;
      }
      if (fin(br[i])) {
        const b = br[i];
